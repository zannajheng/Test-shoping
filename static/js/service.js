...
textarea.addEventListener('input', function() {
 ...
 setTimeout(function () {
    var element = $('.se_body');
    // 检查 scrollHeight 和实际高度
    console.log('scrollHeight:', element.prop('scrollHeight'));
    console.log('clientHeight:', element[0].clientHeight);

    // 确保 scrollTop 设置正确
    element.animate({scrollTop: element.prop('scrollHeight')}, 1500);
}, 500);  // 增加适当的延迟，确保 DOM 渲染完成
    ...
    socket.onmessage = function (event) {
        ...
        if (data.username === username) {
            if (data.type === "text") {
                ...
                $div = '<div class="datetime">' + time + '</div>' + '<div class="raigth_div">' +
                    '<p>' + '<img style="width: 18px; height: 18px" class="Emoji" src="http://127.0.0.1:8000/static/face/' + data.id + '.png" alt="" />' + '</p>' +
                    '</div>'
            }else {
                $div = '<div class="raigth_div">'+'<div class="datetime">' + time + '</div>' + '<div class="raigth_div">'
                    + '<img src="' + data.image_data + '" class="clickable-img">'
                    +'</div>' + '</div>'
            }
        } else {
            if (data.type === "text") {
                $div = '<div class="datetime">' + time + '</div>' + '<div class="line_div">' + '<div class="line_div">' +
                    '<p>' + data.text + '</p>' +
                    '</div>'
            } else if (data.type === "face") {
                $div = '<div class="datetime">' + time + '</div>' + '<div class="line_div">' + '<div class="line_div">' +
       ...
       onclickimg()
            setTimeout(function () {
                var element = document.querySelector('.se_body');
...
function swoll(){
  ...
  const createtime =  () => {
    ...
    function onclickimg(){
     $(".se_body").on("click", ".clickable-img", function() {
